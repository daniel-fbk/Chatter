...
function addEmployee({ first_name, last_name, job_title }) {
  const stmt = db.prepare("INSERT INTO employees (first_name, last_name, job_title) VALUES (?, ?, ?)");
...
function updateEmployee(id, { first_name, last_name, job_title }) {
  const stmt = db.prepare("UPDATE employees SET first_name = ?, last_name = ?, job_title = ? WHERE id = ?");
